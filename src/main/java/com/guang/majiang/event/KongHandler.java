...
/**
 * @ClassName KongHandler
 * @Description TODO
 * @Author guangmingdexin
 * @Date 2021/3/17 15:33
 * @Version 1.0
 **/
public class KongHandler implements EventHandler<MouseEvent> {

    private SpecialEventTask task;

    public KongHandler(SpecialEventTask task) {
        this.task = task;
    }
    ...
    public void handle(MouseEvent event) {
        ...
        // 将 三张牌连在一起
        CardImage[] pong = new CardImage[4];
        pong[0] = task.takeOut;
        ...
        for (int j = 0; j < task.handCards.size() - 1; j++) {
            ...
            if(Double.compare((cur.getX() + 168.0), next.getX()) == 0) {
                for (int k = 0; k <= j; k++) {
                    ImageView temp = task.handCards.get(k).getImageView();
                    temp.setX(temp.getX() + 3 * GlobalConstant.CARD_WIDTH);
                }
            }
        }
