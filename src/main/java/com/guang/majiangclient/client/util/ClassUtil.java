...
public final class ClassUtil {
    ...
    private static void getClassFile(Set<Class<?>> classFiles, File f, String packageName, boolean recursive) {
        ...
        for (File file : files) {
            ...
            if(file.isDirectory()) {
                ...
                try {
                    // 添加到 集合
                    classFiles.add(Class.forName(packageName + "." + className));
                } catch (ClassNotFoundException e) {
                    e.printStackTrace();
                }
            }
     ...
     * @return Class
     */
    public static Class<?> getClassName(Class<?> clazz) {
        Field[] fields = clazz.getDeclaredFields();
        for (Field field : fields) {
            // 获取属性上的注解
            ClassInfo anno = field.getAnnotation(ClassInfo.class);
            if(anno.isDeserializer()) {
                // 需要序列化的 bean
